...
fpath = ".//"
fname = []; title = []; tau_ns  = []; field = []; lwcolor = []; scale = []; target_rho = [];
...
tau_ns.append(140); field.append(11745); lwcolor.append('chocolate'); target_rho.append(0.002);
fname.append("Q250207_07.DSC");title.append("d-NP"  ); scale.append(3);
tau_ns.append(140); field.append(11729); lwcolor.append('olivedrab'); target_rho.append(0.002);
fname.append("Q241204_08.DSC");title.append("d-Tol"); scale.append(1);
tau_ns.append(140); field.append(11745); lwcolor.append('slateblue'); target_rho.append(0.0);
...
input_dict = {
    ...
    'r_start':2,  # [Angst]
    'r_end': 20,    # [Angst]
    'nPoints':50,  # # must be less or equal to experimental number of points, or will be reduced
    'r_scale':'log', #  'log' (log10 base) or 'linear'
    ### things to experimentwith vvvvv
    'BL_damp':0.1, # 0 - full blindspot, 1 - no blindspot
    'Fermi_Contact_a': 0.5, # [Angst] exponential factor on how Aiso depends on distance, probably <1 if to judge from s-type orbital 
    ...
    'unit_lw':0.02, # [MHz] Unitary linewidth for Pake pattern = sigma in gaussian
    'Regularisation':100,
    }
...
def Process_file(input_dict):
    ...
    target_rho_val = input_dict.get('target_rho', 0.0)
    ...
    # Rescale Kernel to solve for Density (rho) instead of P(r)
    # P(r) = rho * 4*pi*r^2 * dr. Solving for rho makes bulk solvent flat.
    dr = np.gradient(xR)
    weights = (xR**2) * dr
    weights /= np.mean(weights) # Normalize weights to keep Regularisation scale consistent
    Kernel_density = Kernel * weights

    # Add "Bulk Solvent" basis function (T=0, Aiso=0)
    Bulk_Signal = Pake(xFreq, 0, 0, tau_us, BL_damp, unit_lw)
    Kernel_density = np.column_stack([Kernel_density, Bulk_Signal])
    
    # Extend xR for the bulk bin for plotting purposes
    xR_plot = np.append(xR, xR[-1] * 1.1)

    ##############################################################################
    # Simple SVD iLT (now solving for density)
    ##############################################################################
    U, S, Vt = np.linalg.svd(Kernel_density)
    
    k = Kernel_density.shape[1]
    ...
    Reg_Zero = input_dict.get('Reg_Zero', Regularisation * 0.05) # Small anchor to zero

    ##############################################################################
    # Non-negative NNLS iLT with Hybrid Regularization (1st-Deriv + Zero-Anchor)
    ##############################################################################
    
    # 1st-derivative matrix L (penalizes changes, enforcing flatness)
    n = Kernel_density.shape[1]
    L = np.zeros((n-1, n))
    for i in range(n-1):
        L[i, i] = -1
        L[i, i+1] = 1
    
    # Constraint for target bulk density
    # Strong weight to "force" the last (bulk) bin
    W_target = 1000.0 * Regularisation
    Target_row = np.zeros(n)
    Target_row[-1] = W_target
    ...
    y_aug = np.concatenate([
        ...
        np.zeros(n-1),
        np.zeros(n),
        [W_target * target_rho_val]
    ])
    
    # dist_nn now represents Proton Density
    dist_nn, residual = nnls(K_aug, y_aug)
    
    # Reconstruct the fit in frequency domain
    nn_Y = Kernel_density @ dist_nn


    return xFreq, yReal, svd_Y, reg_Y, nn_Y, xR_plot, dist_nn, dist_reg, T, Aiso
...
for ii, _ in enumerate(fname):
    ...
    input_dict['target_rho'] = target_rho[ii]
